(* This code was taken from the source code of the ProVerif verifier *)
...
let clear queue = queue.qstart <- None; queue.qend <- None
...
let find queue f =
  let rec existsrec q =
    ...
    | Some q' -> 
        if f q'.elem then q'.elem else existsrec q'.next
  in
  existsrec queue.qstart
